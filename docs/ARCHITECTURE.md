...
`new-cooks` composes digest reconciliation, missing digest/category strength evaluation, kitchen rebuild, and sprint reassignment. Each completed evaluation case checkpoints immediately; interrupted runs resume missing evidence instead of losing completed work. Reassignment compares station, model, digest, and assignment status task-by-task, then preserves a timestamped comparison artifact. A repeated `reassign --model` roster creates a separate candidate plan by default; `--apply-roster` is required to replace the active plan. The independent planning reviewer may still use any eligible installed planning model.
